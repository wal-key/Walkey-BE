"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getPosts = exports.getSessions = exports.getRoutes = exports.getThemes = exports.getUsers = exports.getTables = exports.testConnection = void 0;
const database_1 = __importDefault(require("../config/database"));
/**
 * 데이터베이스 연결 테스트
 */
const testConnection = async (req, res) => {
    try {
        const result = await database_1.default.query('SELECT NOW() as current_time, version() as db_version');
        res.json({
            success: true,
            message: '데이터베이스 연결 성공',
            data: {
                currentTime: result.rows[0].current_time,
                dbVersion: result.rows[0].db_version
            }
        });
    }
    catch (error) {
        console.error('데이터베이스 연결 오류:', error);
        res.status(500).json({
            success: false,
            message: '데이터베이스 연결 실패',
            error: error.message
        });
    }
};
exports.testConnection = testConnection;
/**
 * 모든 테이블 목록 조회
 */
const getTables = async (req, res) => {
    try {
        const result = await database_1.default.query(`
            SELECT table_name 
            FROM information_schema.tables 
            WHERE table_schema = 'public' 
            ORDER BY table_name
        `);
        res.json({
            success: true,
            message: '테이블 목록 조회 성공',
            data: result.rows
        });
    }
    catch (error) {
        console.error('테이블 목록 조회 오류:', error);
        res.status(500).json({
            success: false,
            message: '테이블 목록 조회 실패',
            error: error.message
        });
    }
};
exports.getTables = getTables;
/**
 * 사용자 목록 조회
 */
const getUsers = async (req, res) => {
    try {
        const result = await database_1.default.query('SELECT * FROM users ORDER BY created_at DESC LIMIT 10');
        res.json({
            success: true,
            message: '사용자 목록 조회 성공',
            data: result.rows,
            count: result.rowCount
        });
    }
    catch (error) {
        console.error('사용자 조회 오류:', error);
        res.status(500).json({
            success: false,
            message: '사용자 조회 실패',
            error: error.message
        });
    }
};
exports.getUsers = getUsers;
/**
 * 테마 목록 조회
 */
const getThemes = async (req, res) => {
    try {
        const result = await database_1.default.query('SELECT * FROM themes ORDER BY theme_id');
        res.json({
            success: true,
            message: '테마 목록 조회 성공',
            data: result.rows,
            count: result.rowCount
        });
    }
    catch (error) {
        console.error('테마 조회 오류:', error);
        res.status(500).json({
            success: false,
            message: '테마 조회 실패',
            error: error.message
        });
    }
};
exports.getThemes = getThemes;
/**
 * 산책 루트 목록 조회
 */
const getRoutes = async (req, res) => {
    try {
        const result = await database_1.default.query(`
            SELECT wr.*, t.theme_name, t.color_code
            FROM walk_routes wr
            LEFT JOIN themes t ON wr.theme_id = t.theme_id
            ORDER BY wr.created_at DESC
            LIMIT 10
        `);
        res.json({
            success: true,
            message: '산책 루트 목록 조회 성공',
            data: result.rows,
            count: result.rowCount
        });
    }
    catch (error) {
        console.error('산책 루트 조회 오류:', error);
        res.status(500).json({
            success: false,
            message: '산책 루트 조회 실패',
            error: error.message
        });
    }
};
exports.getRoutes = getRoutes;
/**
 * 산책 세션 목록 조회
 */
const getSessions = async (req, res) => {
    try {
        const result = await database_1.default.query(`
            SELECT ws.*, u.nickname, wr.route_name
            FROM walk_sessions ws
            LEFT JOIN users u ON ws.user_id = u.user_id
            LEFT JOIN walk_routes wr ON ws.route_id = wr.route_id
            ORDER BY ws.start_time DESC
            LIMIT 10
        `);
        res.json({
            success: true,
            message: '산책 세션 목록 조회 성공',
            data: result.rows,
            count: result.rowCount
        });
    }
    catch (error) {
        console.error('산책 세션 조회 오류:', error);
        res.status(500).json({
            success: false,
            message: '산책 세션 조회 실패',
            error: error.message
        });
    }
};
exports.getSessions = getSessions;
/**
 * 커뮤니티 게시글 목록 조회
 */
const getPosts = async (req, res) => {
    try {
        const result = await database_1.default.query(`
            SELECT cp.*, u.nickname, ws.route_id
            FROM community_posts cp
            LEFT JOIN users u ON cp.user_id = u.user_id
            LEFT JOIN walk_sessions ws ON cp.session_id = ws.session_id
            ORDER BY cp.created_at DESC
            LIMIT 10
        `);
        res.json({
            success: true,
            message: '커뮤니티 게시글 목록 조회 성공',
            data: result.rows,
            count: result.rowCount
        });
    }
    catch (error) {
        console.error('커뮤니티 게시글 조회 오류:', error);
        res.status(500).json({
            success: false,
            message: '커뮤니티 게시글 조회 실패',
            error: error.message
        });
    }
};
exports.getPosts = getPosts;
