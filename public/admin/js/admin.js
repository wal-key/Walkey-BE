...
const admin = {
  ...
  bindEvents() {
    ...
    document
      ...
      .addEventListener('submit', (e) => this.handleThemeSubmit(e));
    document
      .getElementById('route-form')
      .addEventListener('submit', (e) => this.handleRouteSubmit(e));
    document
  ...
  async deleteTheme(id) {
    if (
      !confirm(
        '정말 이 테마를 삭제하시겠습니까? 연결된 루트가 있으면 문제가 생길 수 있습니다.'
      )
    )
      return;
    try {
  ...
  async deleteUser(id) {
    if (
      !confirm('정말 이 사용자를 삭제하시겠습니까? 데이터가 모두 삭제됩니다.')
    )
      return;
    try {
  ...
  async fetchMySessions() {
    const tbody = document.querySelector('#my-sessions-table tbody');
    tbody.innerHTML =
      '<tr><td colspan="5" style="text-align:center">로딩 중...</td></tr>';
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.data.length === 0) {
          tbody.innerHTML =
            '<tr><td colspan="5" style="text-align:center">기록이 없습니다.</td></tr>';
          return;
        }
        result.data.forEach((session) => {
          ...
          tr.innerHTML = `
                        <td><small style="color:var(--text-dim)">${session.id}</small></td>
                        <td><strong>${session.route?.name || '알 수 없는 경로'}</strong></td>
                        <td>${session.actual_distance} km</td>
                        <td>${session.actual_duration} 분</td>
                        <td>${startTime}</td>
                    `;
          tbody.appendChild(tr);
        });
      } else {
        tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; color:var(--danger)">${result.message || '데이터를 가져오지 못했습니다.'}</td></tr>`;
      }
    } catch (error) {
      console.error('Failed to fetch my sessions');
      tbody.innerHTML =
        '<tr><td colspan="5" style="text-align:center; color:var(--danger)">오류가 발생했습니다.</td></tr>';
    }
  },
